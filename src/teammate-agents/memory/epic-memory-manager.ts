...
// ===== SIMPLE IN-MEMORY STORAGE (DEFAULT FALLBACK) =====
...
class SimpleInMemoryStorage {
  ...
  async delete(key: string, options?: { partition?: string }): Promise<boolean> {
    const fullKey = options?.partition ? `${options.partition}:${key}` : key;
    return this._data.delete(fullKey);
  }
...
export class EpicMemoryManager extends EventEmitter {
  ...
  private async store(
    ...
    const storeOptions: any = {
      partition: options.partition || options.namespace,
      ttl: options.ttl,
      tags: options.tags,
   ...
   */
  private async findKeys(pattern: string, namespace?: string): Promise<string[]> {
    if (!this.memoryManager) {
      return [];
    }

    try {
      if (typeof this.memoryManager.query === 'function') {
  ...
  async delete(key: string, namespace?: string): Promise<boolean> {
    ...
    try {
      if (typeof this.memoryManager.delete === 'function') {
        await this.memoryManager.delete(key, { partition: namespace });
